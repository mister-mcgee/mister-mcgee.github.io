...
A computer is a "thinking" machine, meaning that it is capable of "choosing" an outcome based on some input or prior state. This is typically accomplished in <Define term="hardware"/> by varying voltages across *switches*. In a modern electronic computer, binary signals are transmitted by encoding zeroes `0` as "low" voltage and ones `1` as "high" voltage, but this wasn't always the case. Early <Define term="electromechanical"/> computers used many different, more exotic techniques to encode and transmit data. Some notable examples include trinary and bi-quinary encodings.
...
It wasn't until after the invention of the *transistor* in 1947 that binary became the dominant method of encoding and transmitting data. The transistor revolutionized computing by dramatically reducing the size and power footprint of computing devices, completely eliminating the need for vacuum tubes and other electromechanical components. Their simplicity was an ideal match for binary.
...
A single bit is capable of representing little more than "off" or "on". If we imagine for a moment, that each bit is a light switch, then we can think of 

import BinaryCombinations from "@components/fun/BinaryCombinations.tsx";

<BinaryCombinations client:only/>

While a single bit is capable of representing little more than "yes" or "no", a *byte* is capable of representing up to 256 different states. If we imagine for a moment, that each bit is a light switch, then we can think of each *state* of a byte as some unique combination of offs and ons. E

A single byte is capable of representing up to 256 different states, usually a number between `0` and `255` inclusive. In order for computers to represent larger or more complicated numbers, they must use *multiple bytes*. For example, most arithmetic on computers is performed using 32-bit (4 bytes) or 64-bit (8 bytes) *integers*. An integer is a whole number (including zero) that can be *signed* (positive or negative) or *unsigned*. Depending on the number of bits and whether it is signed or unsigned, an integer can represent a variety of whole numbers.

<div className="text-xs">
| Width | Signed? | Lower | Upper |
| --- | --- | --- | --- |
| 32-bit (4 bytes) | Yes | $$-2^{31}=-2,147,483,648$$ | $$2^{31}-1=2,147,483,647$$ |
| 32-bit (4 bytes) | No  | $$0$$ | $$2^{32}-1 = 4,294,967,295$$ |
| 64-bit (8 bytes) | Yes | $$-2^{63}=-9,223,372,036,854,775,808$$ | $$2^{63}-1=9,223,372,036,854,775,807$$ |
| 64-bit (8 bytes) | No  | $$0$$ | $$2^{64}-1=18,446,744,073,709,551,615$$ |
</div>
...
Understanding how binary works is fundamental concepts in computer science, and it's the first step toward mastering how computers "think" and operate.
...
<Rubric rubric={rubric} title="Project Rubric"/>
